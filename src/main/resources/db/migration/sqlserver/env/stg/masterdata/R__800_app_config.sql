-- ENVIRONMENT-SPECIFIC master data for: stg
-- Loaded only when app.env=stg. Repeatable + idempotent MERGE.
MERGE INTO app.app_config AS tgt
USING (VALUES
    (N'env.name',                 N'stg'),
    (N'feature.beta-banner',      N'false'),
    (N'notification.webhook-url', N'https://hooks.stg.example.com/db-events')
