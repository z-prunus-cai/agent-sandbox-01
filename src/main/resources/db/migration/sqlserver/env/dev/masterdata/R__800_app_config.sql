-- ENVIRONMENT-SPECIFIC master data for: dev
-- Loaded only when app.env=dev. Repeatable + idempotent MERGE.
MERGE INTO app.app_config AS tgt
USING (VALUES
    (N'env.name',                 N'dev'),
    (N'feature.beta-banner',      N'true'),
    (N'notification.webhook-url', N'https://hooks.dev.internal/db-events')
