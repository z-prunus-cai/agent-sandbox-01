...
结果（硬锚 v3 论文、H100）：相对 v2 在典型形状上约 1.5–2× 加速；BF16 达到约 740 TFLOPs/s、即约 H100 BF16 峰值的 **75%**（这条 75% 利用率是 BF16 的，不是 FP8 的）；FP8 变体达到约 **1.2 PFLOPs/s**（约为 H100 FP8 峰值的 60% 左右）；论文还报告在有离群特征时，其块量化 + 非相干处理比"标准注意力 + 逐张量量化"精度高约 2.6×[^fa3]。这些数字硬绑 v3 + H100 + FP8，不可外推到别的硬件或版本。
...
| v3（2024） | arXiv:2407.08608 | TMA 异步、warp 特化生产者/消费者、ping-pong 调度、FP8 + 非相干处理 | Hopper（H100） | 相对 v2 ~1.5–2×；BF16 ~740 TFLOPs/s(~75% BF16 峰值)、FP8 ~1.2 PFLOPs/s(~60% FP8 峰值)；离群场景块量化比逐张量量化精度约高 2.6×[^fa3] |
