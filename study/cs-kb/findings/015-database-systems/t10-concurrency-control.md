...
S 锁像图书馆里「一本书可以很多人同时看」，X 锁像「有人要在书上写字，别人既不能看也不能同时写」。为什么读读相容、其余都不相容？因为并发控制的正确性尺子是冲突可串行化（03-9），而两个操作「冲突」当且仅当它们访问同一数据项且至少一个是写——读读不冲突所以可以并行，只要沾上写就有冲突、必须排队。一个易错点：光有 S/X 锁**并不足以**保证可串行化，还必须约束「什么时候能加锁、什么时候能放锁」，那就是下面的两阶段锁。此外真实系统里加锁前通常还有一步：读整表要先在**表级**打个意向标记（03-10.3 的意向锁），本节先只看单个数据项。
...
SQL 标准的「SERIALIZABLE 隔离级别」历史上常由 SS2PL 实现，但今天很多引擎（PostgreSQL、Oracle）用 MVCC/SSI 而非纯锁来达到同等或近似效果（03-10.5），所以「SERIALIZABLE 级别」与「用 2PL 实现」不是一回事——规范讲效果，实现各有各的路子。
